class Boid_retained {
  ...
  Boid_retained(Vector inPos, PShape shape_) {
  ...
  public void run(ArrayList<Boid_retained> boids) {
    t += .1;
    flap = 10 * sin(t);
  ...
  void flock(ArrayList<Boid_retained> boids) {
    //alignment
    ...
    for (int i = 0; i < boids.size(); i++) {
      Boid_retained boid = boids.get(i);
      //alignment
